...
| i18n — UI layer | react-i18next wired across all landing page sections and full demo workflow; all app pages (ChatPage, SettingsPage, TopBar) translated; en/zh-CN/ko/es/ja locale files complete | Shipped 2026-03-25 (LP) + 2026-04-15 (app pages + ja) |
| i18n — AI layer | Locale passed in Socket.IO payload; backend injects `"Respond in {language}"` directive into all 5 specialist agent system prompts; dev mode stubs also locale-aware | Shipped 2026-04-15 |
| Dev mode | Settings toggle bypasses all LLM calls via single Orchestrator interceptor; streaming stubs, locale-aware, localStorage-persisted; dev-branch only | Shipped 2026-04-15 |
...
| Conversation history | Persist chat turns to MongoDB; sidebar placeholder in UI | Claude Code | — |
